...
conn.close() 
#|%%--%%| <sLBpadKoVI|G3XVVqqVyt>
new_con = sqlite3.connect(db_path) 
new_cur = new_con.cursor() 

#|%%--%%| <owuU0Axnh9|VrQHSk94YX>
for row in new_cur.execute("SELECT temp_alpha_elim, subst_iupac FROM test_1 ORDER BY temp_beta_elim"):
    print(row) 

#|%%--%%| <owuU0Axnh9|aed4erM6U9>

new_cur.execute("SELECT * FROM test_1 ORDER BY id") 
rows_1 = new_cur.fetchall() 
with open("test_2.csv", "w", newline="") as f: 
    write = csv.writer(f) 
    write.writerow([d[0] for d in new_cur.description])
    write.writerows(rows_1)

#|%%--%%| <aed4erM6U9|owuU0Axnh9>
conn.close() 
